#!/usr/bin/env python3
"""
Quick Phase 2 test to validate improvements without full benchmark
"""
...
async def quick_phase2_test():
    ...
    try:
        ...
        print(f"[OK] Meta-agent orchestration started")
        ...
        print(f"✅ Spawning: {spawning_success} (agents: {initial_agents} → {final_agents})")
        ...
        try:
            ...
            print(f"✅ Task execution: {task_success} ({execution_time:.2f}s)")
        except Exception as e:
            execution_time = time.time() - start_time
            task_success = False
            print(f"❌ Task execution: {task_success} - {str(e)} ({execution_time:.2f}s)")
        ...
        print(f"✅ Decision making: {decision_success} ({decisions_made} decisions)")
        ...
        print(f"✅ Bottleneck analysis: {analysis_success} ({bottlenecks_detected} bottlenecks)")
        ...
        print(f"✅ Knowledge reuse: {knowledge_reuse_improvement} (fitness: {fitness_before:.2f} → {fitness_after:.2f})")
        ...
        if passed_tests >= 4:
            print(f"🎉 PHASE 2 IMPROVEMENTS VALIDATED")
            print(f"✅ Core meta-agent capabilities working")
        else:
...
if __name__ == "__main__":
    success = asyncio.run(quick_phase2_test())
